...
<a href="{{ $href }}" 
   class="flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group {{ $active ? 'bg-biblo-moss text-white shadow-lg shadow-biblo-moss/20' : 'text-biblo-charcoal/60 hover:bg-biblo-oat hover:text-biblo-charcoal' }}">
    
    <div class="{{ $active ? 'text-white' : 'text-biblo-greige group-hover:text-biblo-moss' }} transition-colors">
        {{-- Menggunakan Heroicons atau Feather Icons --}}
        @if($icon == 'layout')
