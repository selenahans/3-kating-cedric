...
<nav class="fixed bottom-0 inset-x-0 z-50 bg-[#FDFBF8] border-t border-biblo-greige/20 shadow-[0_-10px_50px_-12px_rgba(63,69,63,0.08)]">
    <div class="max-w-4xl mx-auto px-6 h-24 flex items-center justify-between">
        ...
        <a href="{{ $prevUrl }}" class="group flex items-center gap-3 text-biblo-charcoal/60 hover:text-biblo-moss transition-all">
        ...
        <div class="hidden md:flex flex-col items-center gap-2">
            <div class="w-32 h-1 bg-biblo-greige/30 rounded-full overflow-hidden">
                <div class="h-full bg-biblo-moss transition-all duration-700 rounded-full" style="width: {{ $progress }}%"></div>
            </div>
            <span class="text-[9px] font-bold text-biblo-moss uppercase tracking-widest">{{ $progress }}% Completed</span>
        </div>

        <a href="{{ $nextUrl }}" class="group flex items-center gap-3 text-biblo-charcoal/60 hover:text-biblo-moss transition-all">
