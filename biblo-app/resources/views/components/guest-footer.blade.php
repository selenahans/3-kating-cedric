<footer class="bg-white border-t border-biblo-greige/20 pt-24 pb-12 px-6">
    <div class="max-w-7xl mx-auto">
        <div class="flex flex-col md:flex-row justify-between items-start gap-16 pb-20">
            <div class="max-w-xs">
                <div class="mb-6">
                    <img src="{{ asset('images/logo/biblo.webp') }}" alt="Biblo Logo" class="h-10 w-auto">
                </div>
                <p class="text-biblo-charcoal/50 text-sm leading-relaxed mb-8">
                    Membangun masa depan lewat lembaran buku dengan cara yang lebih modern dan adiktif (dalam cara yang
                    baik!).
                </p>
            </div>

            <div class="grid grid-cols-2 lg:grid-cols-3 gap-16">
                <div class="flex flex-col gap-4">
                    <h4 class="text-[10px] font-black uppercase tracking-[0.2em] text-biblo-charcoal/30">Layanan</h4>
                    <a href="#" class="text-sm font-semibold hover:text-biblo-moss transition-colors">Perpustakaan</a>
                    <a href="#" class="text-sm font-semibold hover:text-biblo-moss transition-colors">Pet Shop</a>

                </div>

                <div class="flex flex-col gap-4">
                    <h4 class="text-[10px] font-black uppercase tracking-[0.2em] text-biblo-charcoal/30">Perusahaan</h4>
                    <a href="#" class="text-sm font-semibold hover:text-biblo-moss transition-colors">Tentang Kami</a>

                    <a href="#" class="text-sm font-semibold hover:text-biblo-moss transition-colors">Kontak</a>
                </div>

                <div class="hidden lg:flex flex-col gap-4">
                    <h4 class="text-[10px] font-black uppercase tracking-[0.2em] text-biblo-charcoal/30">Ikuti Kami</h4>
                    <div class="flex gap-4">
                        <a href="#"
                            class="group w-10 h-10 bg-biblo-oat rounded-full flex items-center justify-center hover:bg-biblo-sage transition-all duration-300">
                            <svg class="w-5 h-5 text-black group-hover:text-white transition-colors duration-300"
                                fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path
                                    d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c.796 0 1.441.645 1.441 1.44s-.645 1.44-1.441 1.44c-.795 0-1.439-.645-1.439-1.44s.644-1.44 1.439-1.44z" />
                            </svg>
                        </a>

                        <a href="#"
                            class="group w-10 h-10 bg-biblo-oat rounded-full flex items-center justify-center hover:bg-biblo-sage transition-all duration-300">
                            <svg class="w-5 h-5 text-black group-hover:text-white transition-colors duration-300"
                                fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path
                                    d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z" />
                            </svg>
                        </a>
                    </div>
                </div>
            </div>
        </div>

        <div class="pt-8 border-t border-biblo-greige/10 flex flex-col md:flex-row justify-between items-center gap-6">
            <p class="text-[11px] text-biblo-charcoal/40 font-bold uppercase tracking-widest text-center md:text-left">
                © {{ date('Y') }} BIBLO INTERACTIVE. ALL RIGHTS RESERVED.
            </p>
            <div class="flex gap-8 text-[11px] font-bold text-biblo-charcoal/40 uppercase tracking-widest">
                <a href="#" class="hover:text-biblo-moss transition-colors">Privacy</a>
                <a href="#" class="hover:text-biblo-moss transition-colors">Terms</a>
            </div>
        </div>
    </div>
</footer>