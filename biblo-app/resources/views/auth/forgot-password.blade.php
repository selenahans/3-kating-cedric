{{-- <x-auth-layout 
    title="Pulihkan Akun - Biblo"
    icon="🔑"
    slideTitle="Keamanan <br> Adalah Kunci."
    slideDesc="Jangan khawatir, kami akan membantu memulihkan akses ke perpustakaan digitalmu.">

    <div class="flex justify-between items-center mb-6">
        <a href="/">
            <img src="{{ asset('images/logo/biblo.webp') }}" alt="Logo" class="h-7 w-auto">
        </a>
    ...
    <div class="max-w-md mx-auto w-full flex-1 flex flex-col justify-center">
        <div class="mb-8">
            ...
            <p class="text-biblo-charcoal/40 text-xs font-semibold">Masukkan emailmu untuk mendapatkan instruksi reset.</p>
        </div>

        @if (session('status'))
            <div class="mb-4 text-xs font-bold text-green-600 bg-green-50 p-3 rounded-xl border border-green-100">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
            ...
            <div>
                ...
                <input type="email" name="email" required placeholder="example@gmail.com" class="input-box bg-[#F8F9FA]">
                @error('email') <span class="text-[10px] text-red-500 mt-1 font-bold">{{ $message }}</span> @enderror
            </div>

            <button type="submit" 
                class="w-full bg-[#3F453F] hover:bg-black text-white py-4 rounded-2xl font-bold text-sm shadow-xl transition-all transform active:scale-[0.98]">
                Send Reset Link
            </button>
        </form>
        ...
        <a href="#" class="w-full text-center py-3 border border-gray-100 rounded-2xl text-[11px] font-bold text-biblo-charcoal/40 hover:bg-gray-50 transition-all">
            Contact Support Center
        </a>
    </div>

    <p class="mt-8 text-center text-[10px] font-black text-biblo-charcoal/20 tracking-widest uppercase">
