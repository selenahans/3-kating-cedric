    ...
    <div class="max-w-5xl mx-auto space-y-10 pb-20">
        ...
        <header class="flex flex-col md:flex-row justify-between items-end gap-4">
            <div>
                ...
                <h1 class="text-4xl font-extrabold text-biblo-charcoal tracking-tighter">Lumi's <span class="text-biblo-clay">Shop</span></h1>
            </div>
            
            <div class="flex items-center gap-3">
                <div class="bg-white border border-biblo-greige/30 px-6 py-3 rounded-2xl shadow-sm flex items-center gap-3">
                    <div>
        ...
        <nav class="flex overflow-x-auto pb-2 gap-3 no-scrollbar">
            ...
            @foreach($categories as $cat)
                <button class="flex-none px-8 py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all border {{ $cat['active'] ? 'bg-biblo-charcoal text-white border-biblo-charcoal shadow-lg shadow-biblo-charcoal/20' : 'bg-white text-biblo-charcoal/60 border-biblo-greige/20 hover:border-biblo-moss hover:text-biblo-moss' }}">
        ...
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white border border-biblo-greige/20 rounded-[40px] p-6 hover:shadow-2xl hover:shadow-biblo-charcoal/5 transition-all group relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <div class="w-20 h-20 {{ $item['color'] }} rounded-3xl flex items-center justify-center text-4xl mb-6 group-hover:scale-110 group-hover:rotate-3 transition-transform">
                        {{ $item['icon'] }}
                    </div>

                    <div class="space-y-1 mb-6">
                        <h4 class="font-extrabold text-lg text-biblo-charcoal">{{ $item['name'] }}</h4>
        ...
        <footer class="bg-biblo-sage/10 border border-biblo-sage/20 rounded-[45px] p-8 flex flex-col md:flex-row items-center justify-between gap-6">
