<x-app-layout title="Perpustakaan" active="library">
        ...
        <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div
                ...
                <div class="relative z-10">
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] text-biblo-sage mb-2">Koleksiku</p>
                    {{-- Dynamically inject the total book count --}}
                    <h2 class="text-4xl sm:text-5xl font-extrabold tracking-tighter">{{ $totalBooks }} <span
                            class="text-xs sm:text-sm font-bold text-biblo-greige/60 ml-2 uppercase">Buku</span></h2>
                </div>
            ...
            <div
                ...
                <div>
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] text-biblo-charcoal/40 mb-2">Sedang Dibaca
                    </p>
            ...
            <div
                ...
                <div>
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] text-biblo-charcoal/40 mb-2">Selesai
                    </p>
        ...
        <section class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6 lg:gap-10">
            ...
            @empty
                <div class="col-span-full text-center py-10">
                    <p class="text-biblo-charcoal/50">Perpustakaanmu masih kosong.</p>
                </div>
            @endforelse
        </section>
        @endif
            ...
            @forelse($allAchievements as $achievement)
                @if($achievement->type === 'milestone')
                    <div class="bg-gradient-to-br {{ $achievement->color }} rounded-2xl sm:rounded-3xl p-5 sm:p-6 text-biblo-charcoal shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 group">
                        ...
                        <p class="text-xs font-bold opacity-50 uppercase tracking-wide">Terbuka: {{ $achievement->unlocked_at }}</p>
                    </div>
                @else
                    <div class="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-6 border-2 border-biblo-moss/40 shadow-md hover:shadow-xl hover:border-biblo-moss hover:scale-105 transition-all duration-300 group">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex-1">
                                <p class="text-xs font-bold text-biblo-moss uppercase tracking-wider">✓ Tugas Selesai</p>
                        ...
                        <p class="text-xs font-semibold text-biblo-charcoal/50 uppercase tracking-wide">Selesai: {{ $achievement->unlocked_at }}</p>
                    </div>
                @endif
            @empty
                <div class="col-span-full text-center py-16">
                    ...
                    <p class="text-biblo-charcoal/60">Baca buku untuk membuka lencana Perunggu, Perak, dan Emas</p>
                </div>
            @endforelse
        </section>
        @endif
